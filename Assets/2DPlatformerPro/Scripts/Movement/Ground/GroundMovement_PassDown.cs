...
namespace PlatformerPro
{
	...
	public class GroundMovement_PassDown : GroundMovement
	{
		...
		/// </summary>
		protected Collider2D colliderToIgnore;
		...
		void Update()
		{
			if (clearanceTimer > 0) {
				...
				if (clearanceTimer <= 0) character.RemoveIgnoredCollider (colliderToIgnore);
			}
		}
		...
		override public void GainControl()
		{
			...
			if (colliderToIgnore != null) character.RemoveIgnoredCollider(colliderToIgnore);
			colliderToIgnore = character.GroundCollider;
			character.AddIgnoredCollider(colliderToIgnore);
